...
const char sVersion[] = "Open DHCP Server Version 1.70RC Windows Build 1047";
const char htmlStart[] = "<html>\n<head>\n<title>%s</title><meta http-equiv=\"refresh\" content=\"60\">\n<meta http-equiv=\"cache-control\" content=\"no-cache\">\n</head>\n";
//const char bodyStart[] = "<body bgcolor=\"#cccccc\"><table width=\"750\"><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></b></b></td></tr><tr><td align=\"right\"><a target=\"_new\" href=\"http://dhcpserver.sourceforge.net/\">http://dhcpserver.sourceforge.net/</b></b></td></tr></table>";
const char bodyStart[] = "<body bgcolor=\"#cccccc\"><table border=\"0\"><tr><td>\n<table width=\"100%%\" border=\"0\"><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></td></tr><tr><td align=\"right\"><a target=\"_new\" href=\"http://dhcpserver.sourceforge.net\">http://dhcpserver.sourceforge.net</td></tr></table>";
//const char bodyStart[] = "<body bgcolor=\"#cccccc\"><table width=750><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></td></tr><tr><td align=\"right\"><a target=\"_new\" href=\"/scopestatus\">Show Scope Status</td></tr></table>";
//const char scopeStart[] = "<body bgcolor=\"#cccccc\"><table width=750><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></td></tr><tr><td align=\"right\"><a target=\"_new\" href=\"/\">Show Lease Status</td></tr></table>";
//const char bodyStart[] = "<body bgcolor=\"#cccccc\"><table width=750><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></td></tr><tr><td align=\"center\"><font size=\"5\">%s</font></td></tr></table>";
const data4 opData[] =
    {
		{ "SubnetMask", 1, 3 , 1},
		{ "TimeOffset", 2, 4 , 1},
...
//	printArray("userArray", (char*)cfig.userArray);
		...
		if (req->dhcpEntry->rangeInd >= 0)
		{
			...
			if (range->dhcpEntry[ind] == req->dhcpEntry && checkRange(&rangeData, req->dhcpEntry->rangeInd))
			{
				...
				if (!cfig.rangeSet[rangeSetInd].subnetIP[0])
				{
					...
					if (minRange < range->rangeStart)
						minRange = range->rangeStart;

					if (maxRange > range->rangeEnd)
						maxRange = range->rangeEnd;
	...
	for (char k = 0; !iipNew && k < cfig.rangeCount; k++)
	{
		if (checkRange(&rangeData, k))
		{
			...
			{
				//sprintf(logBuff, "Start=%u End=%u", minRange, maxRange);
				//logMess(logBuff, 1);
				rangeFound = true;

				if (cfig.replication == 2)
				{
					for (MYDWORD m = maxRange; m >= minRange; m--)
					{
				...
				{
					for (MYDWORD m = minRange; m <= maxRange; m++)
					{
						int ind = m - range->rangeStart;
...
*/
		...
		req->dhcpEntry->subnetFlg = (req->subnetIP != 0);
		setTempLease(req->dhcpEntry);
		return req->dhcpEntry->ip;
	}
...
MYDWORD sdmess(data9 *req)
{
	...
	{
		req->dhcpp.header.bp_yiaddr = chaddr(req);
	...
	{
		if (req->dhcpp.header.bp_ciaddr && chaddr(req) == req->dhcpp.header.bp_ciaddr)
		{
	...
	else if (req->req_type == DHCP_MESS_REQUEST)
	{
		...
		if (req->server)
		{
			...
			{
				if (req->reqIP && req->reqIP == chaddr(req) && req->dhcpEntry->expiry > t)
				{
					req->resp_type = DHCP_MESS_ACK;
					req->dhcpp.header.bp_yiaddr = req->reqIP;
				}
				else if (req->dhcpp.header.bp_ciaddr && req->dhcpp.header.bp_ciaddr == chaddr(req) && req->dhcpEntry->expiry > t)
				{
					req->resp_type = DHCP_MESS_ACK;
		...
		else if (req->reqIP && req->reqIP == chaddr(req) && req->dhcpEntry->expiry > t)
		{
			req->resp_type = DHCP_MESS_ACK;
			req->dhcpp.header.bp_yiaddr = req->reqIP;
		}
	...
	if (req->dhcpEntry && req->dhcpEntry->rangeInd >= 0)
	{
...
bool detectChange()
{
	...
	while (true)
	{
		...
		if (ret != NO_ERROR)
		{
			...
			{
				sprintf(logBuff, "NotifyAddrChange error...%d", WSAGetLastError());
				logDHCPMess(logBuff, 1);
				break;
				//continue;
			}
...
int MyRecvMess(char *buffer, MYWORD buffsize, SOCKET m_Socket, bool *broadcast, SOCKADDR_IN *remote, socklen_t *sockLen)
{
	...
	INT nResult = WSAIoctl(m_Socket, SIO_GET_EXTENSION_FUNCTION_POINTER,
	...
	if (pCMsgHdr && pCMsgHdr->cmsg_type == IP_PKTINFO)
	{
		...
		//printf("LADDR = %s\n", inet_ntoa(pPktInfo->ipi_addr));
		*broadcast = (pPktInfo->ipi_addr.s_addr == INADDR_NONE);
...
int MyRecvMess(char *buffer, MYWORD buffsize, SOCKET m_Socket, bool *broadcast, SOCKADDR_IN *remote, socklen_t *sockLen)
{
	return recvfrom(m_Socket, buffer, buffsize, 0, (sockaddr*)remote, sockLen);
}
#endif
*/
		...
		switch (op->opt_code)
		{
			...
			case DHCP_OPTION_HOSTNAME:
				...
				if (!strcasecmp(req->hostname, "(none)") || !strcasecmp(req->hostname, cfig.servername))
					req->hostname[0] = 0;
				else if (char *ptr = strchr(req->hostname, '.'))
					*ptr = 0;
